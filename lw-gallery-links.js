;(function($) {
  ...
  // Function to open gallery detail links as a fullscreen gallery instead of a separate page
  var initGalleryLinks = function() {
    ...
    $body.on('click', 'a[href^="/live/galleries/"]', function(e) {
      ...
      if ( $galleryLink.data('fs-gallery') != 'enabled') {

        // Set fullscreen state to enabled so we don't create the gallery again
        $galleryLink.data('fs-gallery', 'enabled');
        ...
        $.get(url, function(images) {
          ...
          $.ajax({
            ...
            $gallery.find('.gallery-images').fsgallery({
              ...
              caption: $gallery.find('.caption'),         // a jQuery selector containing each image caption (must be inside list element)
